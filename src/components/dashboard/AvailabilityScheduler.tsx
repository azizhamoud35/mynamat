...
const formSchema = z.object({
  ...
  const daysDifference = differenceInDays(data.endDate, data.startDate);
  return daysDifference >= 7;
}, {
  message: "Schedule duration must be at least 1 week",
  path: ["endDate"]
});
...
export function AvailabilityScheduler({ onSubmitSuccess, onCancel }: AvailabilitySchedulerProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      startDate: new Date(),
      endDate: addDays(new Date(), 7), // Default to 1 week duration
    },
  });
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      onSelect={(date) => {
                        field.onChange(date);
                        // Update end date to maintain minimum 1-week duration
                        const currentEndDate = form.getValues('endDate');
                        if (date && differenceInDays(currentEndDate, date) < 7) {
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      disabled={(date) => {
                        ...
                        return (
                          date < addDays(startDate, 7) || // Minimum 1 week from start date
                          date < new Date() ||
